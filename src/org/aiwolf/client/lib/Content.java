...
public class Content implements Cloneable {
	...
	//private static final String regAgent = "\\s+(\\[\\d+\\]\\S|ANY)";
	//private static final String regSubject = "^(\\[\\d+\\]\\S|ANY|)\\s*";
	...
	private Content(String input, boolean isForValidation) {
		...
		//System.out.println(trimmed + "**** Content 190");
		Matcher m;
		try {
			// SKIP
	...
	private static Agent toAgent(String s) {
		...
		if (m.find()) {
			if (m.group(1).equals("ANY")) {
				...
				//return Agent.getAgent(Integer.parseInt(agentPattern2.matcher(s)));
