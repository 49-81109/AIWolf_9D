...
public class GameInfo {
	...
	List<Agent> suicideImmoralist; //妖狐追放によって道連れになった背徳者のリスト
	int executedFoxDay; // 妖狐追放によって背徳者の道連れが起きた場合のみ、妖狐追放日が格納される
	Agent guardedAgent;
	List<Vote> voteList;
	...
	public GameInfo() {
		...
		suicideImmoralist = new ArrayList<>();
	}
	 ...
	 * @return
	 */
	public String getAgentName(Agent a) {
//		System.out.println(nameMap.size() + " -> GameInfo 175");
		return nameMap.get(a);
	}
	 ...
	 * <div lang="ja">
	 * 
	 * 昨夜実際に襲撃されたエージェントを返します。人狼以外には知らされません。失敗含む
	 * 
	 * </div>
	 * 
	 * <div lang="en">
	...
	public Map<Agent, String> getNameMap() {
		return nameMap;
	}
	...
	public List<Agent> getLastDeadAgentList() {
		Collections.shuffle(lastDeadAgentList);
	...
	// 基本妖狐呪殺はエージェント視点区別がつかないのでメソッドをコメントアウトした
	/**
	...
	/**
	 * <div lang="ja">妖狐追放による道連れで死亡した背徳者のリストを返す</div>
	 * 
	 * <div lang="en">Returns the immoralist killed with the last fox.</div>
	 * 
	 * @return <div lang="ja">道連れで死亡した背徳者を表す{@code Agent}のリスト</div>
	 * 
	 *         <div lang="en">{@code List<Agent>} representing the immoralist killed with the last fox.</div>
	 */
	public List<Agent> getSuicideImmoralistWithExecutedFox() {
		return suicideImmoralist;
	}
